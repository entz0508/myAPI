...
var isLoggedIn = function (MYSQL_DLA_CONN, appID, os, clientUID, accountID, accessToken, callBack) {
    MYSQL_DLA_CONN.procIsLoggedIn(appID, os, clientUID, accountID, accessToken, function (err, results) {
        var isSuccess = false;
        if (err) {
            PRINT_LOG.setErrorLog("MYSQL_DLA_CONN.procIsLoggedIn, failed db, code:" + -1, err);
        } else if (isNull(results) || (0 >= results.length) || (0 >= results[0].length)) {
            PRINT_LOG.error(__filename, "", "MYSQL_DLA_CONN.procIsLoggedIn, results is null, \t\t [ " + __filename + "]");
        } else {
            var row = results[0][0];
            if (0 === Number(row.RES)) {
                if (Number(userID) === Number(row.USER_ID)) isSuccess = true;
            } else {
                PRINT_LOG.error(__filename, "", "MYSQL_DLA_CONN.procIsLoggedIn, DB error : " + row.RES, ", msg:" + row.MSG);
            }
        }
        callBack(isSuccess);
    });
