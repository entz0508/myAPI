...
module.exports = function(checkConnect, runQS) {
	function MYSQL_SLP_DLA_INFO_CONN() {
		checkConnect(pool, "MYSQL_SLP_DLA_INFO_CONN.setInterval, pool.getConnection");
	}

	MYSQL_SLP_DLA_INFO_CONN.prototype.procWatchdogPing = function(callBack) {
	...
	MYSQL_SLP_DLA_INFO_CONN.prototype.procGetAppRes = function(requestParams, callBack) {
		pool.getConnection(function(err, connection) {
			if (err) return callBack(err);
	...
	MYSQL_SLP_DLA_INFO_CONN.prototype.procGetAppVersion = function(params, callBack) {
		pool.getConnection(function(err, connection) {
			if (err) return callBack(err);
	...
	return MYSQL_SLP_DLA_INFO_CONN;
};