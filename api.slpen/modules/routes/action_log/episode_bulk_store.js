	...
	app.post("/sen/action/episodeBulkStore", ROUTE_MIDDLEWARE.LOGGED_IN_ACCOUNT_WITH_PROFILE, function(req, res) {
		...
		try {
			...
			for (var i = 0; i < epRecord.playHistory.length; i++) {
				...
				MYSQL_SLP_KW_ACTION_LOG_CONN.procAddActionLogEnStartEnd(requestParams, function(err) {
					if (err) {
			...
			// 올라갔음에도 불구하고 isSuccess 가 false 로 나옴 => 비동기 코드잖아요...-_-a...

			PACKET.sendSuccess(req, res, { msg: "success" });
		} catch (catchErr) {
