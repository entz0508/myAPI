...
exports.add_routes = function(app) {
	app.post('/slp.versionInfo.update', ROUTE_MIDDLEWARE.NO_AUTH_APP, function(req, res) {
		...
		try {
			var requestParams = {
				...
				ver: COMMON_UTIL.trim(req.body.ver),
				summit: COMMON_UTIL.trim(req.body.summit),
				forceUpdate: COMMON_UTIL.trim(req.body.force_update)
			};

			if (global.CONFIG.SERVER_INFO.PASSWORD !== requestParams.passwd)
				return PACKET.sendFail(req, res, { MSG: "password unmatch" });

			MYSQL_SLP_PLATFORM_CONN.procAppVerUpdate(requestParams, function(err, results) {
				if (err) {
					PRINT_LOG.setErrorLog("Failed, MYSQL_SLP_PLATFORM_CONN.procAppVerUpdate", err);
