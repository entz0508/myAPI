...
function add_routes(app) {
    ...
    var loginUser = function(req, res, appID, os, clientUID, isGuest, slpAccountID, slpAccountAccessToken, isKidswattsAppLoggedIn) {
        "use strict";

        var API_PATH = req.route.path;
        var kidswattsAppLogin = 0;
        if( isKidswattsAppLoggedIn ) {
            kidswattsAppLogin = 1;
        }

        MYSQL_SLP_DLA_CONN.procLogin(appID, os, clientUID, isGuest, slpAccountID, slpAccountAccessToken, kidswattsAppLogin, function(err, results) {
            if (err) {
                ...
                if (0 !== retV.res) {
                    ...
                    if( COMMON_UTIL.isNull(responseOBJ.ticket_qty) ) {
                        responseOBJ.ticket_qty = 0;
                    }
    ...
    app.post("/sdla.user.login", ROUTE_MIDDLEWARE.DEFAULT, function(req, res){
        ...
        try {
            ...
            if( !COMMON_UTIL.isValidGuest(isGuest) ) {
            ...
            if(0 === isGuest) {
                if(!COMMON_UTIL.isValidSlpAccountID(slpAccountID)) {
                    PRINT_LOG.error(__filename, API_PATH, " error, params slp_account_id");
                    PACKET.sendFail(req, res, ERROR_CODE_UTIL.RES_ERROR_PARAMETER);
                    return;
                }
                if(!COMMON_UTIL.isValidAccessToken(slpAccountAccessToken)) {
                ...
                HTTP_REQUEST_UTIL.httpIsLoggedInSlpAccount(appID, clientUID, slpAccountID, slpAccountAccessToken, function(err, resData){
                    if(0 !== Number(resData.res) ) {
                        ...
                        if(resData.isLoggedIn) {
                            loginUser(req, res, appID, os, clientUID, isGuest, slpAccountID, slpAccountAccessToken, resData.isKidswattsAppLoggedIn);
                        } else {
                ...
                loginUser(req, res, appID, os, clientUID, isGuest, slpAccountID, slpAccountAccessToken, false);
            }
