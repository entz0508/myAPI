...
function add_routes(app) {
    ...
    var sendMailForgotPasswordToken = function( toEmailAddr , newPwd, lang ) {
        var mail = new NODE_MAILER();
        mail.init();

        var subject = "";
        if("kr" === lang) {
            subject = "SLP 비밀번호 변경 안내 메일";
        }
        else {
            subject = "SLP 비밀번호 변경 안내 메일";
        }
    ...
    app.post("/slp.user.account.forgot.password", ROUTE_MIDDLEWARE.AUTH_APP, function(req, res){
        ...
        try {
            ...
            if( !COMMON_UTIL.isEmail(accountEmail)  ) {
                ...
                MYSQL_SLP_ACCOUNT_CONN.procForgotUserAccountPasswordToken(appID, clientUID, CLIENT_IP, accountEmail,
                    function (err, results) {
                        if (err) {
                            ...
                            if( ERROR_CODE_UTIL.RES_SUCCESS !== retV.res) {
                                ...
                                sendMailForgotPasswordToken(row.EMAIL, row.TOKEN, row.COUNTYR);
                            }
                        }
                    });
            }
