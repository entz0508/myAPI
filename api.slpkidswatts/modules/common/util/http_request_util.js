...
var httpIsLoggedInAccountWithProfile = function(appID, clientUID, accountID, accessToken, slpProfileID, callBack){
    isLoggedInSlpAccountWithProfile(appID, clientUID, accountID, accessToken, slpProfileID, function(err, httpResponseJson){
        ...
        if(err || COMMON_UTIL.isNull(httpResponseJson) ) {
            var targetURL = "";
            if( !COMMON_UTIL.isNull(httpResponseJson) && !COMMON_UTIL.isNull(httpResponseJson.url)) {
                targetURL = httpResponseJson.url;
            }
            resData.res = -1;
            resData.msg = " error, http request, isLoggedInSlpAccountWithProfile, [" + targetURL + "], " + __filename;
        } else {
