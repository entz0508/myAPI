...
var pool = mysql.createPool({
	host: global.CONFIG.MYSQL_SLP_NDE.HOST,
	port: global.CONFIG.MYSQL_SLP_NDE.PORT,
	user: global.CONFIG.MYSQL_SLP_NDE.USER,
	password: global.CONFIG.MYSQL_SLP_NDE.PASSWORD,
	database: global.CONFIG.MYSQL_SLP_NDE.DATABASE,
	connectionLimit: global.CONFIG.MYSQL_SLP_NDE.CONNECTION_LIMIT
});
...
module.exports = function(checkConnect, runQS) {
    ...
    MYSQL_SLP_NDE.prototype.procGetOpenUserTokenVal = function(requestParams, callBack) {
        pool.getConnection(function(err, connection) {
            if (err) return callBack(err);
    ...
    */
    ...
    MYSQL_SLP_NDE.prototype.procOpenRefundProduct = function (requestParams, callBack) {
        pool.getConnection(function (err, connection) {
            ...
            var queryStr = " CALL spOpenRefundPermition( " + Number(requestParams.stepAttendID) + " , " +
                connection.escape(requestParams.refundID) + " , " +
    ...
    MYSQL_SLP_NDE.prototype.procOpenStudyStatus = function(requestParams, callBack) {
        pool.getConnection(function(err, connection) {
            if (err) return callBack(err);
            var queryStr = " CALL spOpenStudyStatus( " + Number(requestParams.stepAttendID) + " , "  +
                connection.escape(requestParams.attendStatCD) + " , " +
                Number(requestParams.extPdtID) + " , "  +
                connection.escape(requestParams.attendFrYmd) + " , " +
                connection.escape(requestParams.attendToYmd) + " , " +
                connection.escape(requestParams.bluearkUid) + " ) " ;
            runQS(connection, queryStr, callBack);
        });
    };

    MYSQL_SLP_NDE.prototype.procCallStudyStatusSEQ = function(requestParams, callBack) {
        pool.getConnection(function(err, connection) {
            if (err) return callBack(err);
            var queryStr = " CALL spCallStudyStatusSEQ( " + Number(requestParams.seqID) + " ) ";
            runQS(connection, queryStr, callBack);
        });
    };

    MYSQL_SLP_NDE.prototype.procCallStudyStatusStepAttendID = function(requestParams, callBack) {
        pool.getConnection(function(err, connection) {
            if (err) return callBack(err);
            var queryStr = " CALL spCallStudyStatus( " + Number(requestParams.stepAttendID) + " ) ";
            runQS(connection, queryStr, callBack);
        });
    };

    MYSQL_SLP_NDE.prototype.procCallAllSEQID = function(requestParams, callBack) {
        pool.getConnection(function(err, connection) {
            if (err) return callBack(err);
            var queryStr = " CALL sptestAllSEQ( " + Number(requestParams.accountID) + " ) ";
            runQS(connection, queryStr, callBack);
        });
    };
    // MYSQL_SLP_NDE.prototype.procCallStudyStatus = function(requestParams, callBack) {
    //     pool.getConnection(function(err, connection) {
    //         if (err) return callBack(err);
    //         var queryStr = " CALL spCallStudyStatus( " + Number(requestParams.stepAttendID) + " ) ";
    //         runQS(connection, queryStr, callBack);
