...
if(!isset($_SESSION['user_login']) || !isset($_SESSION['user_name'])) {
	...
	exit;
                ...
                if($userID > 0)
                {
                    $sql = "select a.account_id,a.EMAIL,STAR_POINT,a.COUNTRY,a.SIGNUP_PATH,a.APP_ID,BLOCK,DELETED,a.REG_DATETIME,c.country_name_kr,c.country_name, n.ENC_USER_ID
                                                    from slp_account_db.account_tb a
                                                        left join  slp_platform_db.country_tb c on a.COUNTRY = c.iso_cd
														left join slp_account_db.nde_account_tb n on a.ACCOUNT_ID = n.ACCOUNT_ID
                                                    where a.account_id = $userID";
                    $result = $db->query($sql);
                    $row = $result->fetch_assoc();
					$appID = $row['APP_ID'];		// 1000000007
                }
                ?>
              <!-- page start-->
              <div class="row">
                  <div class="col-sm-6">
                      ...
                      <section class="panel">
                          ...
                          <table class="table">
                              ...
                              <tr>
                                  <td>EMAIL<br/><font size="1px">ENC_USER_ID</font></td>
                                  <td colspan="3">
									<?=$row['EMAIL']?>
									<?
									if($row['ENC_USER_ID'] != ""){
										?>
										<br/><font size="2px"><?=$row['ENC_USER_ID']?></font>
										<?
									}
									?>
