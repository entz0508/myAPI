...
if(!isset($_SESSION['user_login']) || !isset($_SESSION['user_name'])) {
	...
	exit;
                ...
                //echo "<br/>".$wherequery;
				$sql = 'select count(pt.idx) as cnt
						from playlog_tb pt
						left join account_tb act on pt.ACCOUNT_ID = act.ACCOUNT_ID
							where pt.community_id > 0 '.$wherequery;
                //echo "<br/>".$sql;
				$result = $db->query($sql);
				...
				if($allPost > 0){
					...
					$sql = 'select pt.idx, pt.ACCOUNT_ID
									, act.SIGNUP_ID, act.USERNAME, act.SIGNUP_PATH
									, pt.community_id, pt.detail_id
									, ct.praystart, ct.oldbible, ct.newbible, ct.prayend 
									, DATE_FORMAT(pt.REG_DATETIME, "%Y-%m-%d %T") as dt
								from playlog_tb pt
								left join account_tb act on pt.ACCOUNT_ID = act.ACCOUNT_ID
								left join community_tb ct on pt.community_id = ct.communityid and pt.detail_id = ct.id 
                                where pt.community_id > 0 '. $wherequery.'
                                order by pt.IDX desc ' . $sqlLimit;
                    //echo "<br/>".$sql;
					$result = $db->query($sql);
				}
				?>
