...
if(!isset($_SESSION['user_login']) || !isset($_SESSION['user_name'])) {
	...
	exit;
                ...
                //echo "<br/>".$wherequery;
				$sql = 'select count(et.idx) as cnt
						from bookmark_tb et
							left join account_tb act on et.ACCOUNT_ID = act.ACCOUNT_ID
				...
				if($allPost > 0){
					...
					$sql = 'select 
								...
								, et.CID as chapter_id
								, et.VID as verse_id
								, DATE_FORMAT(et.REG_DATETIME, "%Y-%m-%d %T") as dt
								from bookmark_tb et
									left join bible_tb bt on et.BID = bt.id
									left join account_tb act on et.ACCOUNT_ID = act.ACCOUNT_ID
                                where 1 = 1 '. $wherequery.'
                                order by et.IDX desc ' . $sqlLimit;
                    //echo "<br/>".$sql;
					$result = $db->query($sql);
				}
				?>
